...
def _process_lidar(ranges: np.ndarray, max_range: float, normalize: bool = False) -> np.ndarray:
    """
    Sanitize and optionally normalize lidar ranges.

    normalize=False (default): clip to [0, max_range], bad returns → max_range.
                               Values stay in metres — same units as goal/position.
    normalize=True (legacy):   divide by max_range → [0, 1].
    """
    out = np.asarray(ranges, dtype=np.float32).copy()
    out = np.where(np.isfinite(out), out, max_range)
    out = np.where(out < 0, max_range, out)
    if normalize:
...
class PolicyRunner:
    ...
    def __init__(
        ...
        # normalize_lidar: False = raw metres (new default), True = [0,1] (legacy).
        # Old checkpoints without this key default to True for backward compatibility.
        _nl = self.meta.get("normalize_lidar") if self.meta else None
        self.normalize_lidar: bool = bool(int(np.asarray(_nl).flat[0])) if _nl is not None else True
        print(f"[PolicyRunner] normalize_lidar={self.normalize_lidar}  "
              f"({'[0,1]' if self.normalize_lidar else 'raw metres'})", flush=True)
    ...
    def _build_input(self, obs: Observation) -> Tuple[np.ndarray, np.ndarray]:
        ...
        lidar = _process_lidar(obs.lidar_ranges, obs.lidar_max_range, normalize=self.normalize_lidar)
