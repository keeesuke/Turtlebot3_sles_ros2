...
class MPPIPlanner:
    ...
    def debug_plot_no_valid_trajectories(self, start_state, goal, sampled_controls, nominal_controls, all_trajectories, valid_mask):
        ...
        try:
            ...
            self.get_logger().info(f"    Linear velocity only: {linear_velocity_only} ({linear_velocity_only/num_total*100:.1f}%)")
            self.get_logger().info(f"    Angular velocity only: {angular_velocity_only} ({angular_velocity_only/num_total*100:.1f}%)")
            self.get_logger().info(f"    Collision + linear velocity: {collision_linearv} ({collision_linearv/num_total*100:.1f}%)")
            self.get_logger().info(f"    Collision + angular velocity: {collision_angularv} ({collision_angularv/num_total*100:.1f}%)")
            self.get_logger().info(f"    Control only: {control_only} ({control_only/num_total*100:.1f}%)")
...
class HAANavigationNode(Node):
    def __init__(self):
        ...
        try:
            self.get_logger().info("Waiting for /lidar_occupancy_map...")
            # TODO(migration): wait_for_message replaced - handled by subscriber flag
            ...
            self.get_logger().warn("⚠ Timeout waiting for /lidar_occupancy_map. Make sure mapping simulation is running.")
        ...
        try:
            ...
            import time; time.sleep(0.5)
            if self.state_ready:
                self.get_logger().info("✓ Robot state received and processed")
            else:
                self.get_logger().warn("⚠ State message received but not processed yet. Waiting...")
                # Wait a bit more for callback
                timeout_count = 0
                while not self.state_ready and timeout_count < 10:
                    import time; time.sleep(0.1)
                    timeout_count += 1
                if self.state_ready:
                    self.get_logger().info("✓ Robot state processed")
                else:
                    self.get_logger().warn("⚠ State still not ready after waiting")
        except Exception:
            self.get_logger().warn("⚠ Timeout waiting for /gazebo/model_states. Make sure simulation is running.")
        ...
        self.get_logger().info("HAA-only planner node started:")
