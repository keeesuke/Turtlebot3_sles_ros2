#!/usr/bin/env python3
"""
ROS2 Simulated LiDAR Publisher (16-obstacle random configuration with shapes)
Converted from: simulate_lidar_publisher_vector_new.py (ROS1)
"""

import rclpy
from rclpy.node import Node
from sensor_msgs.msg import LaserScan
from gazebo_msgs.msg import ModelStates
from tf_transformations import euler_from_quaternion
import numpy as np
from scipy.stats import truncnorm, truncexpon
import math
import time
import copy


class SimulatedLidarPublisherNew(Node):
    def __init__(self):
        super().__init__('simulate_lidar_publisher')

        # Parameters
        self.declare_parameter('lidar_publish_frequency', 20.0)
        self.declare_parameter('num_lidar_rays', 360)
        self.declare_parameter('lidar_max_distance', 1.0)
        self.declare_parameter('gaussian_probability', 0.91)
        self.declare_parameter('exponential_probability', 0.03)
        self.declare_parameter('uniform_probability', 0.03)
        self.declare_parameter('max_probability', 0.03)

        # Obstacle parameters (16 obstacles): x, y, size, shape
        for i in range(1, 17):
            self.declare_parameter(f'random_x_{i}', 0.0)
            self.declare_parameter(f'random_y_{i}', 0.0)
            self.declare_parameter(f'random_size_{i}', 0.2)
            self.declare_parameter(f'random_shape_{i}', 'rectangle')

        self.publish_frequency = self.get_parameter('lidar_publish_frequency').value
        self.num_rays = self.get_parameter('num_lidar_rays').value
        self.max_distance = self.get_parameter('lidar_max_distance').value
        self.gaussian_probability = self.get_parameter('gaussian_probability').value
        self.exponential_probability = self.get_parameter('exponential_probability').value
        self.uniform_probability = self.get_parameter('uniform_probability').value
        self.max_probability = self.get_parameter('max_probability').value
        self.num_obstacles = 16

        self.sigma = 0.02
        self.lambda_exp = 5

        self.true_robot_state = None
        self.scan_time_i = None

        # Publisher / Subscriber
        self.lidar_publisher = self.create_publisher(LaserScan, '/simulated_scan', 10)
        self.create_subscription(ModelStates, '/gazebo/model_states', self.model_states_callback, 10)

        # Load obstacle data
        self.obstacle_data = self._load_obstacles()
        for i, (x, y, size, shape) in enumerate(self.obstacle_data, 1):
            self.get_logger().info(
                f'Obstacle {i}: pos=({x:.4f}, {y:.4f}), size={size:.4f}, shape={shape}')

        self.true_vertices = self._create_obstacles()
        self._precalculate_edges()

        self.create_timer(1.0 / self.publish_frequency, self.publish_lidar_data)

    def _load_obstacles(self):
        obstacles = []
        for i in range(1, self.num_obstacles + 1):
            x = self.get_parameter(f'random_x_{i}').value
            y = self.get_parameter(f'random_y_{i}').value
            size = self.get_parameter(f'random_size_{i}').value
            shape = self.get_parameter(f'random_shape_{i}').value
            obstacles.append((x, y, size, shape))
        return obstacles

    def _generate_rectangle_vertices(self, cx, cy, size):
        h = size / 2.0
        return [[cx-h, cy-h], [cx+h, cy-h], [cx+h, cy+h], [cx-h, cy+h]]

    def _generate_hexagon_vertices(self, cx, cy, size):
        r = size / 2.0
        return [[cx + r * math.cos(math.radians(i*60)),
                 cy + r * math.sin(math.radians(i*60))] for i in range(6)]

    def _generate_triangle_vertices(self, cx, cy, size):
        base = size
        h = base * math.sqrt(3) / 2.0
        return [[cx - base/2, cy - h/3], [cx + base/2, cy - h/3], [cx, cy + 2*h/3]]

    def _create_obstacles(self):
        obstacles = []
        for x, y, size, shape in self.obstacle_data:
            shape = str(shape).strip().lower()
            if shape == 'rectangle':
                verts = self._generate_rectangle_vertices(x, y, size)
            elif shape == 'hexagon':
                verts = self._generate_hexagon_vertices(x, y, size)
            elif shape == 'triangle':
                verts = self._generate_triangle_vertices(x, y, size)
            else:
                verts = self._generate_rectangle_vertices(x, y, size)
            obstacles.append(verts)
        return obstacles

    def _precalculate_edges(self):
        edges_start_list, edges_end_list = [], []
        for poly in self.true_vertices:
            poly = np.array(poly)
            edges_start_list.append(poly)
            edges_end_list.append(np.roll(poly, -1, axis=0))
        self.edges_start = np.concatenate(edges_start_list, axis=0)
        self.edges_end = np.concatenate(edges_end_list, axis=0)

    def model_states_callback(self, msg):
        try:
            index = msg.name.index('turtlebot3_waffle_pi')
        except ValueError:
            return
        pose = msg.pose[index]
        self.scan_time_i = self.get_clock().now().to_msg()
        q = (pose.orientation.x, pose.orientation.y, pose.orientation.z, pose.orientation.w)
        _, _, ori = euler_from_quaternion(q)
        self.true_robot_state = np.array([pose.position.x, pose.position.y, ori % (2*np.pi)])

    def simulate_lidar(self):
        weights = {
            'gaussian': self.gaussian_probability,
            'exponential': self.exponential_probability,
            'uniform': self.uniform_probability,
            'max': self.max_probability,
        }
        scan_time = self.scan_time_i
        x_r, y_r, theta_r = self.true_robot_state
        scan_robot_state = copy.deepcopy(self.true_robot_state)

        angles = (np.linspace(0.0, 2*np.pi, self.num_rays, endpoint=False, dtype=np.float64)
                  + np.float64(theta_r))
        r_x = np.cos(angles).reshape(-1, 1)
        r_y = np.sin(angles).reshape(-1, 1)

        edge_dx = (self.edges_end[:, 0] - self.edges_start[:, 0]).reshape(1, -1)
        edge_dy = (self.edges_end[:, 1] - self.edges_start[:, 1]).reshape(1, -1)
        qp_x = (self.edges_start[:, 0] - x_r).reshape(1, -1)
        qp_y = (self.edges_start[:, 1] - y_r).reshape(1, -1)

        den = r_x * edge_dy - r_y * edge_dx
        valid_den_mask = np.abs(den) > 1e-8
        t_vals = np.where(valid_den_mask, (qp_x * edge_dy - qp_y * edge_dx) / den, np.inf)
        u_vals = np.where(valid_den_mask, (qp_x * r_y - qp_y * r_x) / den, np.inf)

        valid = (t_vals >= 0) & (u_vals >= 0) & (u_vals <= 1) & valid_den_mask
        t_min = np.min(np.where(valid, t_vals, np.inf), axis=1)
        distances = np.where(t_min > self.max_distance, -1, t_min)

        choices = np.random.choice(
            ['gaussian', 'exponential', 'uniform', 'max'],
            size=self.num_rays,
            p=list(weights.values())
        )
        noisy_distances = distances.copy()

        gm = choices == 'gaussian'
        em = choices == 'exponential'
        um = choices == 'uniform'
        mm = choices == 'max'

        if np.any(gm):
            d = distances[gm]
            result = np.full_like(d, -1.0)
            vm = d != -1.0
            if np.any(vm):
                a = (0.0 - d[vm]) / self.sigma
                b = (self.max_distance - d[vm]) / self.sigma
                result[vm] = truncnorm.rvs(a, b, loc=d[vm], scale=self.sigma)
            noisy_distances[gm] = result

        if np.any(em):
            d = distances[em]
            result = np.full_like(d, -1.0)
            vm = d != -1.0
            if np.any(vm):
                scale_exp = 1.0 / self.lambda_exp
                b_exp = d[vm] / scale_exp
                result[vm] = truncexpon.rvs(b_exp, loc=0, scale=scale_exp)
            noisy_distances[em] = result

        if np.any(um):
            noisy_distances[um] = np.random.uniform(0, self.max_distance, size=um.sum())
        if np.any(mm):
            noisy_distances[mm] = -1.0

        return noisy_distances.tolist(), scan_robot_state, scan_time

    def publish_lidar_data(self):
        if self.true_robot_state is None:
            return
        lidar_readings, scan_state, scan_time = self.simulate_lidar()

        scan_msg = LaserScan()
        scan_msg.header.stamp = scan_time
        scan_msg.header.frame_id = 'base_link'
        scan_msg.angle_min = float(scan_state[2])
        scan_msg.angle_max = 2 * np.pi
        scan_msg.angle_increment = 2 * np.pi / len(lidar_readings)
        scan_msg.range_min = float(scan_state[0])
        scan_msg.range_max = float(scan_state[1])
        scan_msg.ranges = lidar_readings
        self.lidar_publisher.publish(scan_msg)


def main(args=None):
    rclpy.init(args=args)
    node = SimulatedLidarPublisherNew()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()


if __name__ == '__main__':
    main()
