...
class SwitchMPCDiffusionRealWorldNode(Node):
    ...
    def _get_diffusion_trajectory(self):
        """
        Run the diffusion policy for the current state and return
        (world_traj, controls) where world_traj is (H, 5): [x, y, theta, v, w]
        and controls is (H-1, 2): [a, alpha].

        Returns None if lidar or state is not ready.
        """
        ...
        # Extract v, w from trajectory channels 4, 5 if available; otherwise derive
        if action_dim >= 6:
            v_traj = traj_local[:, 4] * self._policy_v_max
            w_traj = traj_local[:, 5] * self._policy_w_max
        else:
            # Derive v from consecutive position deltas in world frame
            ...
            dtheta = np.diff(world_poses[:, 2], prepend=self.theta)
            w_traj = dtheta / dt

        # Build world_traj (H, 5): [x, y, theta, v, w]
        world_traj = np.column_stack([world_poses, v_traj, w_traj])  # (H, 5)

        # Compute acceleration controls [a, alpha] from consecutive velocities
        dt = self.policy_runner.dt
        a_seq     = np.diff(v_traj) / dt     # (H-1,)
        alpha_seq = np.diff(w_traj) / dt     # (H-1,)
        controls = np.column_stack([a_seq, alpha_seq])  # (H-1, 2)

        return world_traj, controls
    ...
    def planning_loop(self, event=None):
        ...
        try:
            ...
            if self.current_planner == "HPA":
                ...
                world_traj, controls = self._get_diffusion_trajectory()
                if world_traj is None:
                    return

                # HAA feasibility check: project one step ahead using first waypoint
                dynamics = TurtlebotDynamics(dt=self.dt)
                haa_check_state = RobotState(
                    x=world_traj[0, 0],
                    y=world_traj[0, 1],
                    theta=world_traj[0, 2],
                    v=float(np.clip(world_traj[0, 3], 0, self.v_limit_hpa)),
                    w=float(np.clip(world_traj[0, 4], -self.omega_limit_hpa, self.omega_limit_hpa)),
                )
